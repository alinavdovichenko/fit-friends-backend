...
export function getCoachTrainingsQuery(state: State): URLSearchParams {
  const { limit, currentPage } = state.CATALOG_DATA;
  const { price, calories, rating, duration } = state.TRAININGS_LIST.filter;
  ...
  if (duration.length) {
    duration.forEach((item) => {
      params.append('trainingDurations', item);
    });
  }
  return params;
}
